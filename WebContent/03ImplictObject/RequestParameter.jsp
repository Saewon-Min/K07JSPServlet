...
/*
전송방식에 상관없이 getParameter()로 폼 값을 받을 수 있다.
만약 값이 입력되지 않았다면 길이가 0인 String객체를 반환하고
파라미터명이 틀린 경우 null을 반환한다.
*/
...
%>
	<!-- 
	위와 같은 방법은 불편하므로 스크립트릿을 분리한 후 HTML영역을 만들어
	JS를 기술한다.
	-->
...
/*
radio는 항목은 여러개지만 선택가능한 항목이 1개이기 때문에
getParameter()를 통해 폼값을 받을 수 있다.
*/
String sexStr = request.getParameter("sex");

/*
checkbox는 2개 이상의 항목을 선택할 수 있으므로 getparameterValues()를 사용한다.
...
if(favo != null){
	for(int i=0; i<favo.length; i++){
		...
		문자열의 마지막 부분에 ,를 제거하기 위해 if문을 통해 처리한다.
		*/
		if(i==favo.length-1){
			favStr += favo[i];
		}else{
...
/*
getParameterNames()
	: 폼값의 이름을 한꺼번에 받아올 때 사용하는
	  메소드로 반환타입은 Enumeration 형이다.
hasMoreElements() 
	: 객체에 남은 폼값의 이름이 있는지 확인한다.
nextElement()
	: 받은 폼 값 중 다음 폼 값의 이름을 반환한다.
*/
...
while(names.hasMoreElements()){
	...
	if(paramName.equals("sex")|| paramName.equals("favorite")){
		...
		// 그 외 나머지는 getParameter()로 읽어온다.
		System.out.println("getParameter()로 읽기");
		out.println("파라미터명을 메소드로 얻어서 값 출력 : <br/>");
