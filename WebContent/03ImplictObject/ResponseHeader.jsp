...
응답헤더 설정하기
	...
	출력되지 않는다면 이는 캐시에 저장된 내용을 출력하고 있기때문이다.
	아래 설정은 캐시를 사용하지 않고 매번 새로고침(F5)한것처럼
	새로운 정보로 갱신하겠다는 응답헤더 설정이다.
*/
// HTTP/1.0에서의 설정(현재는 사용하지 않지만 호환성을 위해 남겨둠)
response.setHeader("Pragma", "no-cache");
// HTTP/1.1에서의 설정(현재 사용중임)
...
응답헤더 추가/설정하기
	add 계열
		: 기존의 헤더명이 존재할 경우 기존의 값을 유지하면서
		계속 추가한다. 없을때는 새롭게 생성한다.
	set 계열
		: 기존 헤더명이 존재할 경우 기존값이 변경된다.
		없을때는 새롭게 생성한다.
*/
...
response.setIntHeader("numberOfCrew", 99); // 기존값에 새로운 값을 덮어쓰기(수정)
...
<body>
	...
	/*
	getHeaderNames()
		: 응답헤더의 이름을 얻어온다. 반환타입은 Collection형이다.
	*/
	Collection<String> headerNames = response.getHeaderNames();
	
	// foreach문을 통해 얻어온 응답 헤더를 전체 출력한다.
