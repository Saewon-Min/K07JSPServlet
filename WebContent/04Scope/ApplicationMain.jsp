...
- 링크를 통해 새로운 페이지로 이동하거나, 포워드, 인클루드 등 모든 페이지에서
	영역이 공유된다. 이런 페이지들은 하나의 웹 어플리케이션 내에 존재하기 때문이다.
*/
...
// util패키지의 날짜를 sql패키지의 날짜로 변경한 후 출력
java.sql.Date mySqlDate = new Date(myUtilDate.getTime());
System.out.println("mySqlDate = "+mySqlDate);

// DTO객체 생성
MemberDTO first = new MemberDTO("Kim","1111","김씨",mySqlDate);
MemberDTO second = new MemberDTO("Park","2222","박씨",null);
MemberDTO third = new MemberDTO("Lee","3333","이씨",null);

// List 계열의 컬렉션 생성 후 DTO객체를 저장한다.
...
// application 영역에 리스트와 Map 컬렉션 저장
application.setAttribute("lists", lists);
application.setAttribute("maps", maps);

// page 영역, request 영역에 각각 속성 저장
...
<body>
	...
	*/
	...
	<!-- 
	외부파일 include 
		: include 지시어를 통해 페이지를 삽입하면 같은 페이지로 간주되므로
		page, request 영역이 둘 다 공유하게 된다.
	-->
