...
request.setAttribute("requestMember1", 
		new MemberDTO("Min","1234","민세원",null));
...
<body>
	...
	<ul>
		<!-- 기본 클래스인 경우 별도의 형변환 필요없음 -->
		<li>Integer 타입 : <%=request.getAttribute("requestNumber") %></li>
		<li>String 타입 : <%=request.getAttribute("requestString") %></li>
		<%
		// 개발자가 정의한 클래스는 형변환 필요함
		MemberDTO member = (MemberDTO)request.getAttribute("requestMember1");
		%>
		<li>MemberDTO 타입 : 아이디 : <%=member.getId()%>,
								비번 : <%=member.getPass()%>,
								이름 : <%=member.getName()%>,
	...
	<h2>리퀘스트 영역에 저장된 속성 삭제 후 출력하기</h2>
	<%
		request.removeAttribute("NoKeyValue");// 키값이 틀려도 에러 발생 안함
	...
	<%-- 
	최초 요청 된 페이지와 포워드로 전달된 페이지는 request영역을 공유하게 된다.
	URL주소창에는 최초 요청된 페이지의 경로명이 보여지지만, 출력되는 페이지는
	새롭게 전달된 페이지의 내용을 출력하게 된다.
	그러므로 사용자는 페이지 이동을 인지하지 못하고, 내부적으로는 
	페이지전달(포워드)이 되는 것이다. 
	쿼리스트링 형태로 파라미터도 전달할 수 있다.
	 --%>
	...
	<%-- 
	sendRedirect()는 JS의 location.href와 동일한 기능으로 새로운 페이지에 대한
	요청을 하게 된다. 이 경우 request영역은 공유되지 않는다. 웹브라우저의
	주소창에는 새롭게 요청된 페이지의 경로명이 보여지게 된다.	
	--%>
