...
<body>
	...
	1. 쿠키 객체 생성 
		: new Cookie(쿠키명, 쿠키값)
		쿠키명을 설정하는 setName()메소드가 별도로 존재하지 않는다.
		그러므로 생성자를 통해 쿠키명을 설정해야한다.
	*/
	Cookie cookie = new Cookie("UserID","KOSMO");
	
	/*
	2. 쿠키의 적용경로 설정
		: 쿠키 설정시 쿠키가 적용될 경로를 설정한다.
		아래의 설정은 컨텍스트 루트 경로를 사용하므로 웹어플리케이션
		전체에서 사용 가능한 쿠키를 생성하게 된다.
		초 단위임 
		60초*60분 = 1시간으로 설정됨
	*/
	...
	*/
	...
	/*
	3. 쿠키의 유효시간 설정(초단위)
		: 유효시간을 설정하지 않을 경우 웹브라우저를 닫을 때 
		자동으로 삭제된다. 그 외에는 설정한 시간만큼 유지된다.
	*/
	...
	<%-- 
		...
		즉, 새로고침하거나 다른 페이지에 새로운 요청이 있을때 서버는
		쿠키를 확인 할 수 있게된다.
	 --%>
	<h2>쿠키를 설정하는 현재페이지에서 쿠키값 확인하기</h2>
	<%
	// request 객체를 통해 쿠키값을 얻어온다. 여러개의 값이므로 배열로 반환한다.
