...
<body>
	...
	<!--  
		[set태그의 속성]
			var 
				: 변수명으로 문자열만 가능
			value 
				: 값을 설정함. 표현식, EL, 일반값 모두 가능함
			scope 
				: 영역을 지정하여 변수를 저장함
					4가지 영역을 지정할 수 있고, 
					기본값은 가장 좁은 page영역
		
		즉, set 태그를 통해 변수를 저장하면 지정된 영역에 저장된다.
	-->
	<h2>set태그</h2>
	...
	<h3>set 태그로 설정한 데이터 표현식으로 출력</h3>
	<ul>
		<li>
	...
	<!--  
	EL로 영역의 속성을 읽어올때는 xxScope를 생략하면 가장 좁은
	...
	<!--  
	인자 생성자를 통해 DTO객체를 생성한 후 request영역에 속성을 저장한다.
	-->
	<h3>인자 생성자로 설정</h3>
	...
	<!--  
	영역에 저장될때 Object형으로 자동형변환 되므로 사용시 해당
	자료형으로 형변환해야한다.
	
	형변환을 해야 getter를 호출할 수 있다.
	-->
	<h3>JSP코드로 출력</h3>
	<ul>
		<li>
	...
	<!--  
	EL로 출력시에는 별도의 형변환이 필요없고, 속성명의 주복이 없다면
	내장 객체를 생략할 수 있으므로 편리하다.
	-->
	<h3>EL로 출력</h3>
	<ul>
	...
	<!--  
	1. 변수 설정. new를 통해 객체를 생성한 후 setAttribute()으로 영역에 저장
	-->
	<c:set var="defaultMember" value="<%=new MemberDTO() %>" 
		scope="request"/>
	
	<!--  
	2. 생성된 변수의 멤버변수 값을 지정함. setter를 통한 값 설정
	-->
	...
	<!--  
	List 계열의 컬렉션은 배열처럼 순서를 보장하고, 인덱스로 접근되므로
	객체에 접근시 인덱스를 사용한다.
	-->
	<h2>List 컬렉션 이용하기</h2>
	<%
	...
	<h4>1단게 : set 태그로 리스트 계열 컬렉션 설정</h4>
	...
	정지훈에 대한 정보가 출력됨
	-->
	<h4>target 및 property 속성으로 미 설정 후 출력</h4>
	<ul>
	...
	<c:set target="${list[0] }" property="id" value="BTS"></c:set>
	<c:set target="${list[0] }" property="pass" value="7777"></c:set>
	<c:set target="${list[0] }" property="name" value="방탄소년단"></c:set>
	...
	<h4>1단계 : set태그로 맴 컬렉션 설정</h4>
	<c:set var="map" value="<%=map %>" scope="request"></c:set>

	<h4>2단계 : target 및 property 속성으로 속성값 설정</h4>
	<c:set target="${map.args }" property="id" value="owg"></c:set>
	<c:set target="${map.args }" property="pass" value="7777"></c:set>
	<c:set target="${map.args }" property="name" value="옹성우"></c:set>
