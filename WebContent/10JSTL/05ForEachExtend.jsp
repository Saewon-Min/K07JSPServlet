...
<body>
	<!--  
	forEach태그의 두번째 문법 : 확장 for문의 형태
		주로 배열이나 컬렉션에서 값을 순차적으로 접근하고 싶을때
		사용한다. varStatus에 지정된 값들 중 index 속성은
		0부터 시작한다. 필수 속성으로 items 와 var가 있다.
	-->
	...
	set태그의 변수 선언한다. 즉, 영역에 저장해야 JSTL에서 
	사용할 수 있다.
	-->
	...
	<!--  
	확장형 : forEach 태그의 varStatus 속성
	- index 
		: 현재 루프내에서의 인덱스를 표시한다. 0부터 시작한다.
	- current
		: 현재 루프의 실제 요소를 반환한다.
	- count
		: 실제 반복횟수를 반환한다. 1부터 시작한다.
	-->
	...
	<ul>
		<!-- index는 컬렉션 크기-1 까지 존재한다. -->
		<c:forEach begin="0" end="${lists.size()-1 }" var="i" >
		
		<li>
			<!-- index를 사용하므로 컬렉션 변수 []를 사용한다. -->
			아이디 : ${lists[i].id },
			패스워드 : ${lists[i].pass },
			이름 : ${lists[i].name }
		</li>
	...
	<%-- 
	루프내에서 객체를 전달할 변수 map을 통해 맵의 key와 value를 알아낼 수 있다.
	key => ${map명.key}
	value => ${map명.value}
