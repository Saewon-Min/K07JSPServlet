...
<body>
	...
	<!--  
	JSTL의 import태그
		- include 액션 태그와 같은 기능으로 request영역을 공유한다.
			서로 다른 페이지이므로 page영역은 공유되지 않는다.
		- url 속성에 절대경로 설정시 컨텍스트 루트명은 제외한다.
	-->
	<h2>import 태그</h2>
	<c:set var="requestVar" value="리퀘스트영역" scope="request" />

	<h3>var속성 미지정 - 해당 위치에 바로 삽입됨(include 액션태그와 동일함)</h3>
	<!-- 아래는 param의 value를 지정하는 두가지 방법 
		- 태그 사이에 값을 지정
		- value속성에 값을 지정 -->
	...
	-->
	<h3>import 선언은 위에서 , 삽입은 아래서 처리</h3>
	${contents }


	<h3>외부(원격)에 있는 내용 삽입하기</h3>
	<iframe src="./inc/ImportNate.jsp"  frameborder="1" style="width:100%; height:500px" > 
	</iframe>
