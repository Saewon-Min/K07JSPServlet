...
<body>
	...
	- HTML과 비슷하게 태그로 표현하지만, XML은 개발자가 직접
		태그를 정의할 수 있음
	- HTML과 같이 보여주는 목적이 아닌, 데이터를 전달하는 것이
		주 목적임. 주로 서버간 통신에 사용됨.
	
	
	파싱(Parsing)
	- 주어진 데이터를 분석하여, 원하는 데이터의 특정 패턴이나
		특정 순서로 추출하는 것이 가능한것을 말한다.
	- 즉, XML이나 HTML 등으로 구성된 데이터를 분석하여
		사용자가 원하는 부분만 추출하는 것을 말한다.
	
	-->
	
	<!--  
		XML 데이터를 set태그를 통해 변수로 저장함
	-->
	<c:set var="booklist" >
		<booklist>
			<book>
	...
	<!-- 해당 데이터를 파싱할때 사용. XML 데이터를 blist로 지정함 -->
	<x:parse xml="${booklist }" var="blist"/>
	
	
	<!--  
	출력하고자 하는 XML의 노드에 접근하여 해당 엘리먼트를 출력함	
	XML의 노드에 접근할때 아래오 같이 경로 형태로 접근하는 것을 
	XPath라고 함.
	
	book[1] : 첫번째 엘리먼트를 표시함. 배열처럼 0부터 시작하지 않음
	-->
	<h4>파싱1</h4>
	...
	<table border="1">
		...
		<!--  
		forEach 태그를 통해 선택한 노드의 개수만큼 반복함
		반복되는 엘리먼트는 var에서 지정한 item에 저장됨
		-->
